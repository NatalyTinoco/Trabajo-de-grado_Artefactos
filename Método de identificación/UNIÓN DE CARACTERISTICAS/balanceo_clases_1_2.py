...
file = r'C:\Users\Nataly\Documents\Trabajo-de-grado_Artefactos\Método de identificación\UNIÓN DE CARACTERISTICAS\DM_sSIM.xlsx'
...
datosT = [[datos['sSIMN'][i] for i in range(h[0])],[datos['Clase '][i] for i in range(h[0])]]
#    
#    
#    
#    datosT=[[],[],[]]
...
#        datosT[0].append(datosRE1['EntropiaSF'][p])
        datosT[0].append(datosRE1['sSIMN'][p])
        datosT[1].append(1)
        c +=1
d=0
while d <= 153:
...
#        datosT[0].append(datosNO1['EntropiaSF'][pq])
        datosT[0].append(datosNO1['sSIMN'][pq])
        datosT[1].append(1)
        d+=1
...
#        datosT[0].append(datosRE2['EntropiaSF'][p])
        datosT[0].append(datosRE2['sSIMN'][p])
        datosT[1].append(1)
        c +=1
d=0
while d <= 154:
...
#        datosT[0].append(datosNO2['EntropiaSF'][pq])
        datosT[0].append(datosNO2['sSIMN'][pq])
        datosT[1].append(1)
        d+=1
...
consolidado = pd.DataFrame(datos).to_excel('binarioDM_sSIMN.xlsx')
