...
datosmix = pd.concat([datosRE,datosNO], ignore_index=True)  
datosT = [[],[],[]]
sampleRE = []
sampleNO=[]
...
c=0
while c <= 154:
    p= random.randint(0, len(datosRE))
...
d=0
while d <= 153:
    pq= random.randint(0, len(datosNO))
...
datos={'clase':[1 for i in range(len(datosT[0]))],'EntropiaSF':datosT[0],'sSIMN': datosT[0]}
