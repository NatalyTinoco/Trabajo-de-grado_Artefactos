...
contraste=np.zeros((329,1))
energia=np.zeros((329,1))
homogeneidad=np.zeros((329,1))
correlacion=np.zeros((329,1))
disimi=np.zeros((329,1))
ASM=np.zeros((329,1))
entropia=np.zeros((329,1))
i=0               
...
for x in range ((len(contraste[:,0]))):
    hoja[table[0]+ str (x+4)]=contraste[x,a]
    hoja[table[2]+ str (x+4)]=energia[x,a]
    hoja[table[4]+ str (x+4)]=homogeneidad[x,a]
    hoja[table[6]+ str (x+4)]=correlacion[x,a]
    hoja[table[8]+ str (x+4)]=disimi[x,a]
    hoja[table[10]+ str (x+4)]=ASM[x,a]
    hoja[table[12]+ str (x+4)]=entropia[x,a]
