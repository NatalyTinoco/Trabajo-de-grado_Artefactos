...
def comparacionhistRE(img,img2,segmenta,a):
    ...
    if a==0:
        for z in range((3)):
           img[:,:,z]=(img[:,:,z]*(imgseg))
        hista = cv2.calcHist([img], [0, 1, 2], None, [8, 8, 8],[1, 256, 0, 256, 0, 256])
        imgseg=cv2.imread(segmenta,0)
        imgseg1=imgseg*-1
        imgseg1 = cv2.normalize(imgseg1, None, 0, 1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC3)
        for z in range((3)):
           img2[:,:,z]=(img2[:,:,z]*(imgseg1))
        histb = cv2.calcHist([img2], [0, 1, 2], None, [8, 8, 8],[1, 256, 0, 256, 0, 256])
    else:
        img=img*imgseg
        hista = cv2.calcHist([img], [0], None, [256],[1, 256])
        imgseg=cv2.imread(segmenta,0)
        imgseg1=imgseg*-1
        imgseg1 = cv2.normalize(imgseg1, None, 0, 1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC3)
        img2=img2*imgseg1
    ...
    if a==0:
        ...
        for l in range(3):
         ...
         hist2 = cv2.calcHist([img2], [0], None, [256], [1, 256])
